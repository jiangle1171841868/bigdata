...
object SpackStreamingSocket {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val inputDS1: ReceiverInputDStream[String] = ssc.socketTextStream("node03", 9999, StorageLevel.MEMORY_ONLY)
    //todo 从多个数据源读取数据
    val inputDS2: ReceiverInputDStream[String] = ssc.socketTextStream("node02", 8888, StorageLevel.MEMORY_AND_DISK)
    //将两个DStream合并
    val inputDS: DStream[String] = inputDS1.union(inputDS2)
