...
object StreamingKafkaTotalAndCheck {

  //大小写转化ctr+shift+u
  val CHECK_POINT_PATH: String = "/datas/spark/checkpoint/0000"
  ...
  def main(args: Array[String]): Unit = {
    ...
    val ssc: StreamingContext = StreamingContext.getActiveOrCreate(
      ...
      //todo 当流式应用第一次运行的时候(检查点目录不存在 会创建StreamingContext实例对象) 创建检查点目录 检查点存在的时候 会获取之前的状态  应用停顿时间间隔的数据 会在启动之后处理
      CHECK_POINT_PATH,
      () => ({
  ...
  def processData(ssc: StreamingContext) = {
    ...
    /**
      ...
      *      - 里面的泛型就是value的数据类型
      */
    val stateDStream: DStream[(Int, Double)] = dataDStream.updateStateByKey(
      (values: Seq[Double], state: Option[Double]) => {
        ...
        //d.返回最新的销售额  Option类型 有数据就返回Some 没有数据就返回Nobe
        Some(total)
      }
    )
