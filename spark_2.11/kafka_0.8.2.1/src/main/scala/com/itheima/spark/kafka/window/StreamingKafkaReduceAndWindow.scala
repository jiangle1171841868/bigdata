...
object StreamingKafkaReduceAndWindow {

  //大小写转化ctr+shift+u
  val CHECK_POINT_PATH: String = "/datas/spark/checkpoint/00003"
  ...
  def processData(ssc: StreamingContext) = {
    ...
    val stateDStream: DStream[(Int, Long)] = kafkaDStream
      //a.能对RDD处理就不对DStream处理 不能对RDD处理在对DStream处理
      .transform { rdd =>
      rdd
        //b.处理之前必先过滤(清楚数据结构)
    ...
    /// TODO: 窗口分析 将窗口函数个reducer放在一起使用
    /// TODO: 每隔4秒统计前6秒的数据
    /**
      * def reduceByKeyAndWindow(
      * reduceFunc: (V, V) => V,  //聚合value
      * windowDuration: Duration,  //窗口间隔
      * slideDuration: Duration  //滑动间隔
      * ): DStream[(K, V)]
      */
    val countDStream: DStream[(Int, Long)] = stateDStream.reduceByKeyAndWindow(

      //value的数据需要指定类型
      (v1: Long, v2: Long) => v1 + v2,
      //(v1: Long, v2: Long) => v1 - v2,
      Seconds(STREAMING_WINDOW_INTERVAL),
      Seconds(STREAMING_WINDOW_SLIDE_INTERVAL)
    )
    ...
    countDStream.foreachRDD { (rdd, time) =>
